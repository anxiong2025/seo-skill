...
<details>
<summary><strong>完整 SEO 优化清单（点击展开）</strong></summary>
...
- [ ] GA4 分析代码已安装
- [ ] Search Console / Bing Webmaster 验证已部署
- [ ] 无误加的 `noindex`，Sitemap 已配置

### 技术 SEO

- [ ] `robots.txt` 存在且未屏蔽重要页面
- [ ] Sitemap 包含所有可索引页面
- [ ] 每个页面有 canonical、HTTPS、无混合内容
- [ ] 无死链、无重定向链
- [ ] 移动端适配、全局导航、面包屑

### On-Page SEO

- [ ] `<title>` 唯一、50-60 字符、含关键词
- [ ] `<meta description>` 唯一、120-155 字符
- [ ] 每页一个 `<h1>`，层级无跳级
- [ ] 图片有描述性 `alt`
- [ ] 内链充足、锚文本描述性强
- [ ] URL 小写、连字符、层级 ≤ 3
...
- [ ] Organization / BreadcrumbList / WebSite JSON-LD
- [ ] 文章页 Article、FAQ 页 FAQPage、产品页 Product
- [ ] 所有 JSON-LD 语法正确、内容与页面一致

### OG / 社交

- [ ] `og:title`、`og:description`、`og:image`（1200×630）齐全
- [ ] Twitter Card 完整、OG 图片文件存在
...
- [ ] 产品有独特描述 + Product Schema
- [ ] 分类页有内容、缺货页保留、分面导航无重复

### 本地 SEO

- [ ] NAP 全站一致、LocalBusiness JSON-LD
- [ ] 联系页有地图、多地点有独立落地页
...
- [ ] 语义化 landmark、对比度达标、表单有 label
- [ ] 键盘可达、焦点指示器可见

### HTML 验证

- [ ] DOCTYPE、charset 正确、无重复标签
- [ ] 无占位文本、无废弃标签

### JS 渲染 / SSR

- [ ] SSG 或 SSR 配置、关键内容服务端渲染
- [ ] SPA 用 history API、有 noscript 回退

### 国际化

- [ ] `lang` 属性正确、hreflang 双向 + x-default

### GEO（AI 引用优化）

- [ ] AI 爬虫已放行、`llms.txt` 已创建
- [ ] 首段直接回答核心问题、数据标注来源
- [ ] 语义标记完整（`<article>`、`<figure>`、`<time>`、`<dfn>`）
- [ ] 实体一致、Schema 含 sameAs
- [ ] FAQ/列表/表格格式利于 AI 提取

### 高级 SEO

- [ ] 精选摘要格式优化
- [ ] 年份和 dateModified 为最新
- [ ] 无侵入式弹窗

> 详见 [SKILL.md](SKILL.md) 完整 150+ 项检查。
